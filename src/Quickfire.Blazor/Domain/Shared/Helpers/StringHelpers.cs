...
namespace Quickfire.Blazor.Domain.Shared.Helpers
{
    public static class StringHelper
    {
        ...
        public static string BuildThumbnailPath(Attachment attachment)
        {
            ...
            var sanitizedLocalPath = attachment.LocalPath.Replace("\\", "/").Trim('/');
            if (string.IsNullOrEmpty(sanitizedLocalPath))
            {
                return string.Empty;
            }

            var thumbName = $"{Path.ChangeExtension(attachment.HashedFileName, null)}_thumb.jpg";
            var dataLocalPath = sanitizedLocalPath.EndsWith("/.data", StringComparison.OrdinalIgnoreCase)
                ? sanitizedLocalPath
                : $"{sanitizedLocalPath}/.data";

            var dataWebPath = $"/{dataLocalPath}/{thumbName}";
            var absoluteRoot = FileStorageResolverAccessor.Resolver.ResolvedRoots.AbsoluteRoot;
            var dataFsPath = Path.Combine(absoluteRoot, dataLocalPath.Replace("/", Path.DirectorySeparatorChar.ToString()), thumbName);
            if (File.Exists(dataFsPath))
            {
                return dataWebPath;
            }

            var legacyWebPath = $"/{sanitizedLocalPath}/{thumbName}";
            var legacyFsPath = Path.Combine(absoluteRoot, sanitizedLocalPath.Replace("/", Path.DirectorySeparatorChar.ToString()), thumbName);
            if (File.Exists(legacyFsPath))
            {
        ...
        )
        {
            ...
            var resolver = FileStorageResolverAccessor.Resolver;

            if (returnUrl)
            {
                return resolver.BuildPublicUrl(attachment);
            }

            if (returnAbsolute)
            {
                return resolver.BuildAbsolutePath(attachment);
            }

            if (returnFolder)
            {
                return resolver.BuildMappedFolderPath(attachment);
            }

            return resolver.BuildMappedFilePath(attachment);
        }
        ...
        public static string FormatDateTimeDifference(DateTime? date)
        {
            ...
            else // Older dates
            {
                ...
                // If more than 30 days old, just display the date
                if (daysAgo > 30)
                {
        ...
        public static string FormatDateDifference(string dateString, TimeZoneInfo? timeZone = null)
        {
            ...
            return FormatDateDifference(date, timeZone);
        }
        public static string FormatDateDifference(DateTime date, TimeZoneInfo? timeZone = null)
        {
            ...
            var localDate = TimeZoneInfo.ConvertTimeFromUtc(utcDate, effectiveTimeZone);
            var nowLocal = TimeZoneInfo.ConvertTimeFromUtc(DateTime.UtcNow, effectiveTimeZone);
            var timeDifference = nowLocal - localDate;
            var totalSeconds = timeDifference.TotalSeconds;

            if (localDate.Date == nowLocal.Date) // Today
            {
                if (totalSeconds >= 0)
                {
                    if (totalSeconds < 3600)
            ...
            else if (localDate.Date == nowLocal.Date.AddDays(-1)) // Yesterday
            {
                return $"Yesterday at {localDate.ToString("h:mmtt")}";
            }
            else if (localDate.Date < nowLocal.Date) // Older dates
            {
                int daysAgo = (int)(nowLocal.Date - localDate.Date).TotalDays;
                return $"{daysAgo} days ago at {localDate.ToString("h:mmtt")}";
            }
            else if (localDate.Date == nowLocal.Date.AddDays(1)) // Tomorrow
            {
                return $"Tomorrow at {localDate.ToString("h:mmtt")}";
            }
            else // Future dates
            {
                int daysAhead = (int)(localDate.Date - nowLocal.Date).TotalDays;
                return $"In {daysAhead} Days at {localDate.ToString("h:mmtt")}";
            }
        }
        ...
        public static string NormalizePhoneNumber(string phoneNumber)
        {
            ...
            // If number starts with 1 and is 11 digits, use as is
            // If number is 10 digits, add "1" prefix
            // Otherwise, use as is
        ...
        {
            if (Enum.IsDefined(typeof(Quickfire.Blazor.Domain.Renewals.Models.SubmissionStatus), statusInt))
            {
                return ((Quickfire.Blazor.Domain.Renewals.Models.SubmissionStatus)statusInt).ToString();
            }
            else
            {
